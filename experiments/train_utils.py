...
def make_summary(summaries, prefix):
    epoch_summary = {
        prefix
        + "/average_return": np.asarray(
            [sum(episode["reward"]) for episode in summaries]
        ).mean(),
        prefix
        + "/average_episode_length": np.asarray(
...
def evaluate(agent, train_env, logger, config, steps):
    ...
    if config.render_episodes:
        videos = list(
            map(
                lambda episode: episode.get("image"),
        ...
        if config.observation_type in ["rgb_image", "binary_image"]:
            videos = list(
                map(
                    lambda episode: episode.get("observation"),
                    evaluation_episodes_summaries[: config.render_episodes],
                )
            )
            logger.log_video(
...
def train(config, agent):
    ...
    print("+++++++")
    train_env = env_wrappers.make_env(
        config.environment,
